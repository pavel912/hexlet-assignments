...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> firstNameValidator = ctx.formParamAsClass("firstName", String.class)
                .check(UserController::isNotEmptyStringOrNull, "Имя не должно быть пустым");

        Validator<String> lastNameValidator = ctx.formParamAsClass("lastName", String.class)
                .check(UserController::isNotEmptyStringOrNull, "Фамилия не должна быть пустой");

        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(EmailValidator.getInstance()::isValid, "Неверный формат email");
        ...
        Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
                ...
                .check(value -> value != null && value.length() >= 4, "Пароль должен быть не короче 4 символов");
    ...
    private static boolean isNotEmptyStringOrNull(String value) {
        if (value != null) {
            if (!value.isEmpty()) {
                return true;
            }
